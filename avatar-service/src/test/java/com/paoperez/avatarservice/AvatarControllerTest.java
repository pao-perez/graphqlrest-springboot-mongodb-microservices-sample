...
class AvatarControllerTest {
    ...
    void getAllAvatars_shouldReturnOk() throws Exception {
        final Avatar avatarA =
                Avatar.builder().id("A").userName("userA").imageId("imageIdA").build();
        final Avatar avatarB = Avatar.builder().id("B").userName("userB").build();
        final Collection<Avatar> data = ImmutableList.of(avatarA, avatarB);
        final Avatars avatars = Avatars.builder().data(data).build();
        when(service.getAllAvatars()).thenReturn(data);
    ...
    void getAvatar_whenExistingId_shouldReturnOk() throws Exception {
        ...
        this.mockMvc
                .perform(get("/avatars/{id}", existingId).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
    ...
    void getAvatar_whenNonexistingId_shouldReturnNotFound() throws Exception {
        ...
        this.mockMvc
                .perform(
                        get("/avatars/{id}", nonExistingId).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.status").value(HttpStatus.NOT_FOUND.name()))
                .andExpect(jsonPath("$.message")
                        .value(String.format("Avatar with id %s not found.", nonExistingId)));
    ...
    void createAvatar_whenNonexistingUserName_shouldReturnCreated() throws Exception {
        ...
        final Avatar newAvatar =
                Avatar.builder().userName(nonExistingUserName).imageId(imageId).build();
        final String createdId = "A";
        final Avatar createdAvatar = Avatar.builder().id(createdId).userName(nonExistingUserName)
    ...
    void createAvatar_whenExistingUserName_shouldReturnConflict() throws Exception {
        ...
        final Avatar newAvatar =
                Avatar.builder().userName(existingUserName).imageId("imageIdA").build();
        when(service.createAvatar(newAvatar))
        ...
        this.mockMvc
                ...
                .andExpect(jsonPath("$.message").value(String
                        .format("Avatar with userName %s already exists.", existingUserName)));
    ...
    void createAvatar_whenBlankFields_shouldReturnBadRequest() throws Exception {
        ...
        this.mockMvc
                ...
                .andExpect(jsonPath("$.status").value(HttpStatus.BAD_REQUEST.name()))
                .andExpect(
                        jsonPath("$.message").value(containsString("userName must not be blank")))
                .andExpect(
                        jsonPath("$.message").value(containsString("imageId must not be blank")));
    ...
    void updateAvatar_whenExistingId_shouldReturnNoContent() throws Exception {
        ...
        this.mockMvc
                .perform(put("/avatars/{id}", existingId).contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(updateAvatar)))
    ...
    void updateAvatar_whenExistingUserName_shouldReturnConflict() throws Exception {
        ...
        final String existingUserName = "userA";
        final Avatar currentAvatar = Avatar.builder().id(currentId).userName(existingUserName)
                .imageId("imageIdA").build();
        doThrow(new AvatarAlreadyExistsException(existingUserName)).when(service)
                .updateAvatar(currentId, currentAvatar);
    ...
    void updateAvatar_whenNonexistingId_shouldReturnNotFound() throws Exception {
        final String nonExistingId = "Z";
        final Avatar nonExistingAvatar =
                Avatar.builder().id(nonExistingId).userName("userA").imageId("imageIdA").build();
        doThrow(new AvatarNotFoundException(nonExistingId)).when(service)
                .updateAvatar(nonExistingId, nonExistingAvatar);
    ...
    void deleteAvatar_whenExistingId_shouldReturnNoContent() throws Exception {
        ...
        this.mockMvc
                .perform(
                        delete("/avatars/{id}", existingId).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isNoContent());
    ...
    void deleteAvatar_whenBlankId_shouldReturnBadRequest() throws Exception {
        ...
        this.mockMvc
                .perform(delete("/avatars/{id}", blankId).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.status").value(HttpStatus.BAD_REQUEST.name()))
                .andExpect(jsonPath("$.message").value(containsString("must not be blank")));
    ...
    void deleteAvatar_whenNonexistingId_shouldReturnNotFound() throws Exception {
        ...
        this.mockMvc
                .perform(delete("/avatars/{id}", nonExistingId)
