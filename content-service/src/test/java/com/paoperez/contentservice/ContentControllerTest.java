...
class ContentControllerTest {
  ...
  void getAllContents_shouldReturnOk() throws Exception {
    ...
    this.mockMvc
        .perform(get("/contents").contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(content().string(objectMapper.writeValueAsString(contents)));
  ...
  void getContent_whenExistingId_shouldReturnOk() throws Exception {
    ...
    this.mockMvc
        .perform(get("/contents/{id}", existingId).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(content().string(objectMapper.writeValueAsString(existingContent)));
  ...
  void getContent_whenNonexistingId_shouldReturnNotFound() throws Exception {
    ...
    when(service.getContent(nonExistingId)).thenThrow(new ContentNotFoundException(nonExistingId));

    this.mockMvc
        .perform(get("/contents/{id}", nonExistingId).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.status").value(HttpStatus.NOT_FOUND.name()))
        .andExpect(
            jsonPath("$.message")
                .value(String.format("Content with id %s not found.", nonExistingId)));
  ...
  void getContent_whenBlankId_shouldReturnBadRequest() throws Exception {
    ...
    this.mockMvc
        .perform(get("/contents/{id}", blankId).contentType(MediaType.APPLICATION_JSON))
  ...
  void updateContent_whenNonexistingId_shouldReturnNotFound() throws Exception {
    ...
    final Content paramContent =
        new Content.Builder()
            ...
            .created(LocalDateTime.now().toString())
            .build();
    doThrow(new ContentNotFoundException(nonExistingId))
        .when(service)
        .updateContent(nonExistingId, paramContent);

    this.mockMvc
        .perform(
            put("/contents/{id}", nonExistingId)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(paramContent)))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.status").value(HttpStatus.NOT_FOUND.name()))
  ...
  void deleteContent_whenExistingId_shouldReturnNoContent() throws Exception {
    ...
    this.mockMvc
        .perform(delete("/contents/{id}", existingId).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isNoContent());
  ...
  void deleteContent_whenBlankId_shouldReturnBadRequest() throws Exception {
    ...
    this.mockMvc
        .perform(delete("/contents/{id}", blankId).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.status").value(HttpStatus.BAD_REQUEST.name()))
        .andExpect(jsonPath("$.message").value(containsString("must not be blank")));
  ...
  void deleteContent_whenNonexistingId_shouldReturnNotFound() throws Exception {
    final String nonExistingId = "Z";
    doThrow(new ContentNotFoundException(nonExistingId)).when(service).deleteContent(nonExistingId);

    this.mockMvc
        .perform(delete("/contents/{id}", nonExistingId).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.status").value(HttpStatus.NOT_FOUND.name()))
        .andExpect(
            jsonPath("$.message")
