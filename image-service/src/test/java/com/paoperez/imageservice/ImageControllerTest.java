...
class ImageControllerTest {
    ...
    void getAllImages_shouldReturnOk() throws Exception {
        final Image imageA = Image.builder().id("A").name("imageA").url("/path/to/imageA")
                .alt("Image A").width(150).height(150).build();
        final Image imageB = Image.builder().id("B").name("imageB").url("/path/to/imageB")
                .alt("Image B").width(150).height(150).build();
        final Collection<Image> data = ImmutableList.of(imageA, imageB);
        final Images images = Images.builder().data(data).build();
        when(service.getAllImages()).thenReturn(data);
    ...
    void getImage_whenExistingId_shouldReturnOk() throws Exception {
        ...
        final Image existingImage = Image.builder().id(existingId).name("imageA")
                .url("/path/to/imageA").alt("Image A").width(150).height(150).build();
        when(service.getImage(existingId)).thenReturn(existingImage);

        this.mockMvc
                .perform(get("/images/{id}", existingId).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(content().string(objectMapper.writeValueAsString(existingImage)));
    ...
    void getImage_whenNonexistingId_shouldReturnNotFound() throws Exception {
        ...
        this.mockMvc
                .perform(get("/images/{id}", nonExistingId).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.status").value(HttpStatus.NOT_FOUND.name()))
                .andExpect(jsonPath("$.message")
                        .value(String.format("Image with id %s not found.", nonExistingId)));
    ...
    void createImage_whenNonexistingUrl_shouldReturnCreated() throws Exception {
        ...
        final Image newImage = Image.builder().name(name).url(nonExistingUrl).alt(alt).width(width)
                .height(height).build();
        final String createdId = "A";
        final Image createdImage = Image.builder().id(createdId).name(name).url(nonExistingUrl)
                .alt(alt).width(width).height(height).build();
    ...
    void createImage_whenExistingUrl_shouldReturnConflict() throws Exception {
        ...
        final Image newImage = Image.builder().name("imageA").url(existingUrl).alt("Image A")
        ...
        this.mockMvc
                ...
                .andExpect(jsonPath("$.message")
                        .value(String.format("Image with url %s already exists.", existingUrl)));
    ...
    void updateImage_whenExistingId_shouldReturnNoContent() throws Exception {
        final String existingId = "A";
        final Image updateImage = Image.builder().id(existingId).name("imageA")
                .url("/path/to/updated/image").alt("Image A").width(150).height(150).build();

        this.mockMvc
                .perform(put("/images/{id}", existingId).contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(updateImage)))
    ...
    void updateImage_whenExistingUrl_shouldReturnConflict() throws Exception {
        ...
        final String existingUrl = "/path/to/existing/image";
        final Image currentImage = Image.builder().id(currentId).name("imageA").url(existingUrl)
                .alt("Image A").height(150).width(150).build();
        doThrow(new ImageAlreadyExistsException(existingUrl)).when(service).updateImage(currentId,
                currentImage);
    ...
    void updateImage_whenBlankFields_shouldReturnBadRequest() throws Exception {
        ...
        this.mockMvc
                ...
                .andExpect(jsonPath("$.message").value(containsString("url must not be blank")))
                .andExpect(jsonPath("$.message")
                        .value(containsString("width must be a positive number")))
                .andExpect(jsonPath("$.message")
                        .value(containsString("height must be a positive number")));
    ...
    void updateImage_whenNonexistingId_shouldReturnNotFound() throws Exception {
        final String nonExistingId = "Z";
        final Image nonExistingImage = Image.builder().id(nonExistingId).name("imageA")
                .url("/path/to/imageA").alt("Image A").height(150).width(150).build();
        doThrow(new ImageNotFoundException(nonExistingId)).when(service).updateImage(nonExistingId,
                nonExistingImage);
    ...
    void deleteImage_whenExistingId_shouldReturnNoContent() throws Exception {
        ...
        this.mockMvc
                .perform(delete("/images/{id}", existingId).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isNoContent());
    ...
    void deleteImage_whenBlankId_shouldReturnBadRequest() throws Exception {
        ...
        this.mockMvc
                .perform(delete("/images/{id}", blankId).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.status").value(HttpStatus.BAD_REQUEST.name()))
                .andExpect(jsonPath("$.message").value(containsString("must not be blank")));
    ...
    void deleteImage_whenNonexistingId_shouldReturnNotFound() throws Exception {
        ...
        this.mockMvc
                .perform(delete("/images/{id}", nonExistingId)
