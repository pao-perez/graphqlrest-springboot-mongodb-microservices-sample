...
class ImageControllerTest {
  ...
  @MockBean private ImageService service;
  ...
  void getAllImages_shouldReturnOk() throws Exception {
    ...
    this.mockMvc
        .perform(get("/images").contentType(MediaType.APPLICATION_JSON))
  ...
  void getImage_whenBlankId_shouldReturnBadRequest() throws Exception {
    ...
    this.mockMvc
        .perform(get("/images/{id}", blankId).contentType(MediaType.APPLICATION_JSON))
  ...
  void createImage_whenNonexistingUrl_shouldReturnCreated() throws Exception {
    ...
    final Image newImage =
        Image.builder().name(name).url(nonExistingUrl).alt(alt).width(width).height(height).build();
    final String createdId = "A";
    final Image createdImage =
        Image.builder()
            .id(createdId)
            .name(name)
            .url(nonExistingUrl)
  ...
  void updateImage_whenExistingUrl_shouldReturnConflict() throws Exception {
    ...
    final Image currentImage =
        Image.builder()
            ...
            .width(150)
            .build();
    doThrow(new ImageAlreadyExistsException(existingUrl))
        .when(service)
        .updateImage(currentId, currentImage);

    this.mockMvc
        .perform(
            put("/images/{id}", currentId)
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void updateImage_whenNonexistingId_shouldReturnNotFound() throws Exception {
    ...
    final Image nonExistingImage =
        Image.builder()
            ...
            .width(150)
            .build();
    doThrow(new ImageNotFoundException(nonExistingId))
        .when(service)
        .updateImage(nonExistingId, nonExistingImage);

    this.mockMvc
        .perform(
            put("/images/{id}", nonExistingId)
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void deleteImage_whenNonexistingId_shouldReturnNotFound() throws Exception {
    ...
    this.mockMvc
        .perform(delete("/images/{id}", nonExistingId).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.status").value(HttpStatus.NOT_FOUND.name()))
        .andExpect(
            jsonPath("$.message")
