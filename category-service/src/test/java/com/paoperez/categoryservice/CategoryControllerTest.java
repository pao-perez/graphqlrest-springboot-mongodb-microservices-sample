...
class CategoryControllerTest {
  ...
  void getCategory_whenExistingId_shouldReturnOk() throws Exception {
    ...
    this.mockMvc.perform(get("/categories/{id}", existingId)
  ...
  void getCategory_whenNonexistingId_shouldReturnNotFound() throws Exception {
    ...
    when(service.getCategory(nonExistingId)).thenThrow(
        new CategoryNotFoundException(nonExistingId));

    this.mockMvc.perform(get("/categories/{id}", nonExistingId)
        .contentType(MediaType.APPLICATION_JSON))
  ...
  void getCategory_whenBlankId_shouldReturnBadRequest() throws Exception {
    ...
    this.mockMvc.perform(get("/categories/{id}", blankId)
        .contentType(MediaType.APPLICATION_JSON))
  ...
  void createCategory_whenExistingName_shouldReturnConflict() throws Exception {
    ...
    when(service.createCategory(newCategory)).thenThrow(
        new CategoryAlreadyExistsException(existingName));
    ...
    this.mockMvc
        ...
        .andExpect(jsonPath("$.status")
          .value(HttpStatus.CONFLICT.name()))
        .andExpect(jsonPath("$.message")
  ...
  void updateCategory_whenExistingName_shouldReturnConflict() throws Exception {
    ...
    .when(service).updateCategory(currentId, currentCategory);
  ...
  void updateCategory_whenNonexistingId_shouldReturnNotFound() throws Exception {
    ...
    .when(service).updateCategory(nonExistingId, nonExistingCategory);
  ...
  void deleteCategory_whenExistingId_shouldReturnNoContent() throws Exception {
    ...
    this.mockMvc.perform(delete("/categories/{id}", existingId)
        .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isNoContent());
  ...
  void deleteCategory_whenBlankId_shouldReturnBadRequest() throws Exception {
    ...
    this.mockMvc.perform(delete("/categories/{id}", blankId)
        .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.status")
          .value(HttpStatus.BAD_REQUEST.name()))
  ...
  void deleteCategory_whenNonexistingId_shouldReturnNotFound() throws Exception {
    final String nonExistingId = "Z";
    doThrow(new CategoryNotFoundException(nonExistingId))
    .when(service).deleteCategory(nonExistingId);

    this.mockMvc.perform(delete("/categories/{id}", nonExistingId)
        .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.status")
          .value(HttpStatus.NOT_FOUND.name()))
